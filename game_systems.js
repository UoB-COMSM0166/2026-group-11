...
function isWaveFullySpawned() {
  const wave = currentLevelWaves[currentWaveIndex];
  if (!wave) return true;
  return currentStageIndex >= wave.stages.length - 1 && currentStageQueue.length === 0;
}
...
function checkWaveAdvance() {
  ...
  if (currentWaveIndex >= currentLevelWaves.length - 1) return;

  if (waveClearedTimer === 0) {
    waveClearedTimer = frameCount;
  }

  if (frameCount - waveClearedTimer > 90) {
