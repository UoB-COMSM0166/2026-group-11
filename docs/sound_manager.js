...
let lastRequestedBgmKey = null;
let bgmPlayPendingKey = null;
let blockedBgmKey = null;
let bgmAttemptSerial = 0;
...
function isBgmPlaying(key) {
  if (!gameSoundReady || !gameSoundPools.bgm) return false;

  const pool = gameSoundPools.bgm[key];
  if (!pool || !pool.sounds || pool.sounds.length === 0) return false;

  const audio = pool.sounds[0];
  return !!(audio && !audio.paused && !audio.ended);
}

function isBgmPlayPending(key) {
  return bgmPlayPendingKey === key;
}

function isBgmAutoplayBlocked(key) {
  return blockedBgmKey === key;
}

function resetBgmAttemptState(key) {
  bgmAttemptSerial++;
  if (!key || bgmPlayPendingKey === key) {
    bgmPlayPendingKey = null;
  }
  if (!key || blockedBgmKey === key) {
    blockedBgmKey = null;
  }
}
...
function playBgm(key, options = {}) {
  lastRequestedBgmKey = key;
  if (!gameSoundReady) return false;
  if (isGameAudioMuted()) return false;
  // Autoplay blocked guard prevents draw-loop retry spam.
  if (blockedBgmKey === key && !options.force) return false;

  // BGM uses a single pooled track per scene and loops until the scene changes.
  const bgmPool = gameSoundPools.bgm;
  if (!bgmPool) return false;
  ...
  try {
    const attemptId = ++bgmAttemptSerial;
    bgmPlayPendingKey = key;
    blockedBgmKey = null;
    ...
    if (p && typeof p.catch === "function") {
      p.then(() => {
        if (bgmAttemptSerial !== attemptId) return;
        bgmPlayPendingKey = null;
        blockedBgmKey = null;
      }).catch((err) => {
        if (bgmAttemptSerial !== attemptId) return;
        bgmPlayPendingKey = null;
        blockedBgmKey = key;
        if (typeof currentSceneBgm !== "undefined" && currentSceneBgm === key) {
