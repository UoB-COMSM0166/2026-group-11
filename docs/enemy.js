...
class Enemy {
  ...
  initBossFragments() {
    ...
    const minDist = 260 * scale;
    const maxDist = 620 * scale;
    const jitter = 50 * scale;
    const sourcePath = Array.isArray(this.bossShowcasePath) ? this.bossShowcasePath : this.path;
    // Fragments start at the death center, then fly toward nearby route samples.
    const sampledFragments = this.sampleBossFragmentsNearPath(sourcePath, minDist, maxDist, jitter);
    const radius = Math.max(minDist, this.w * 0.78);

    const targets = sampledFragments.length === 4 ? sampledFragments : [
      { x: this.bossDeathCenterX + radius, y: this.bossDeathCenterY, angleOffset: 0 },
      { x: this.bossDeathCenterX - radius, y: this.bossDeathCenterY, angleOffset: PI * 0.5 },
      { x: this.bossDeathCenterX, y: this.bossDeathCenterY + radius, angleOffset: PI },
      { x: this.bossDeathCenterX, y: this.bossDeathCenterY - radius, angleOffset: PI * 1.5 }
    ];
  ...
  sampleBossFragmentsNearPath(sourcePath, minDist, maxDist, jitter) {
    if (!Array.isArray(sourcePath) || sourcePath.length < 2) return [];

    // Sampling the path keeps fragments near gameplay space without adding extra path data.
    const candidates = [];
    const step = 36 * this.getSafeUiScale();
    for (let i = 0; i < sourcePath.length - 1; i++) {
      ...
      for (let j = 0; j <= samples; j++) {
        ...
        const x = a.x + dx * t;
        const y = a.y + dy * t;
        const d = Math.hypot(x - this.bossDeathCenterX, y - this.bossDeathCenterY);
        if (d >= minDist && d <= maxDist) {
          candidates.push({ x, y, distance: d });
        }
      }
    }
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const point = candidates[pickedIndex];
      const angle = random(0, TWO_PI);
      const offset = random(35 * this.getSafeUiScale(), jitter);
      fragments.push({
        x: point.x + cos(angle) * offset,
        y: point.y + sin(angle) * offset,
        angleOffset: i * HALF_PI
      });
    }
  ...
  updateBossFragmenting() {
    ...
    const returnStartElapsed = Math.max(0, this.fragmentingDuration - this.fragmentReturnDuration);

    for (let frag of this.bossFragments) {
      // Return every fragment before reviving the boss.
      if (elapsed >= returnStartElapsed) {
        this.ensureBossFragmentReturning(frag, elapsed);
      }
    ...
    const allReturned = this.bossFragments.length > 0 && this.bossFragments.every(frag => frag && frag.phase === "returned");
    if (allReturned) {
      // Wait for every fragment to return before reviving the Boss.
  ...
  assignBossFragmentRoamTarget(frag, elapsed) {
    ...
    frag.roamTargetX = target ? target.x : frag.targetX;
    frag.roamTargetY = target ? target.y : frag.targetY;
  ...
  ensureBossFragmentReturning(frag, elapsed) {
    ...
    frag.returnStartedFrame = Math.min(elapsed, Math.max(0, this.fragmentingDuration - this.fragmentReturnDuration));
  ...
  updateFragmentStunAbility() {
    ...
    const now = this.getBossLogicFrame();
    if (now - this.fragmentLastStunFrame < this.fragmentStunCooldown) return;

    const fragmentTarget = this.findHighestValueTowerNearLandedFragment();
    if (!fragmentTarget) return;

    const targetTower = fragmentTarget.tower;
    const sourceFragment = fragmentTarget.fragment;
    this.lastBossTargetX = targetTower.x;
    this.lastBossTargetY = targetTower.y;
    this.bossAttackFlashTimer = 14;
    this.bossAttackEffectType = "stun";
    // Only roaming fragments can attack; projectiles keep the hit source readable.
    // Fragment projectiles use stun only to avoid removing too many towers during split.
    if (typeof spawnBossProjectile === "function") {
      const didFire = spawnBossProjectile({
        type: "stun",
  ...
  findHighestValueTowerNearLandedFragment() {
    if (typeof towers === "undefined" || !Array.isArray(towers) || towers.length === 0) return null;
    if (!Array.isArray(this.bossFragments)) return null;

    // Only roaming fragments can attack towers; outbound and returning phases are visual movement.
    const range = this.bossAttackRange || 320 * this.getSafeUiScale();
    let bestTower = null;
    ...
    for (const frag of this.bossFragments) {
      if (!frag || frag.phase !== "roaming") continue;
    ...
    return bestTower ? { tower: bestTower, fragment: bestFragment } : null;
  }
  ...
  displayBody() {
    ...
    if (this.isBoss) {
      ...
      ellipse(this.x, this.y, this.w * 1.05 * pulse, this.h * 1.05 * pulse);
      stroke(auraInner[0], auraInner[1], auraInner[2], auraInner[3]);
      strokeWeight(2 * scale);
      ellipse(this.x, this.y, this.w * 1.22 * pulse, this.h * 1.22 * pulse);
      pop();
    }
    ...
    if (this.isBoss && this.bossPhase === "fragmenting" && Array.isArray(this.bossFragments)) {
      if (typeof bossEffectImages !== "undefined" && bossEffectImages.vortex && typeof drawBossVfxImage === "function") {
        const remaining = Math.max(0, this.fragmentingDuration - (this.getBossLogicFrame() - this.fragmentingStartedFrame));
        const finalPulse = remaining <= 45 ? 1 + (45 - remaining) / 45 * 0.55 : 1;
      ...
      for (let frag of this.bossFragments) {
        ...
        image(enemyImg, frag.x, frag.y, frag.size, frag.size);
        noTint();
        noFill();
        stroke(frag.landed ? 255 : 255, frag.landed ? 120 : 150, frag.landed ? 210 : 45, frag.landed ? 120 : 170);
        strokeWeight(2 * scale);
        ellipse(frag.x, frag.y, frag.size * (1.05 + 0.05 * sin(frag.pulse)), frag.size * (1.05 + 0.05 * sin(frag.pulse)));
        if (frag.landed) {
  ...
  displayHealthBar() {
    ...
    const scale = this.getSafeUiScale();
    const barW = this.isBoss ? this.w * 0.9 : 50 * scale;
    const barH = this.isBoss ? 10 * scale : 6 * scale;
    const barX = this.x - barW * 0.5;
    const barY = this.isBoss ? this.y - this.h * 0.55 - 18 * scale : this.y - 46 * scale;
