...
window.CannonBomb = class CannonBomb {
  ...
  explode() {
    ...
    lasers.push(new IceBurstEffect(this.x, this.y, this.splashRadius));
    this.dead = true;
  }
  ...
  display() {
    ...
    fill(c.main[0], c.main[1], c.main[2], 150);
    ellipse(this.x, this.y, 44 * uiScale, 24 * uiScale);

    fill(c.hit[0], c.hit[1], c.hit[2], 245);
    beginShape();
    vertex(this.x, this.y - 20 * uiScale);
    vertex(this.x + 23 * uiScale, this.y);
    vertex(this.x, this.y + 20 * uiScale);
    vertex(this.x - 23 * uiScale, this.y);
    endShape(CLOSE);

    fill(c.dark[0], c.dark[1], c.dark[2], 170);
    ellipse(this.x - 15 * uiScale, this.y + 6 * uiScale, 26 * uiScale, 10 * uiScale);
...
window.ArrowProjectile = class ArrowProjectile {
  ...
  update() {
    ...
    if (this.t >= 1) {
      ...
      lasers.push(new CannonHitEffect(this.x, this.y));
  ...
  display() {
    ...
    for (let i = 0; i < this.trail.length; i++) {
      let p = this.trail[i];
      fill(c.dark[0], c.dark[1], c.dark[2], p.a * 0.7);
      ellipse(p.x, p.y, (13 + i * 2.1) * uiScale, (9 + i * 1.3) * uiScale);
      fill(c.main[0], c.main[1], c.main[2], p.a * 0.58);
      ellipse(p.x, p.y, (8 + i * 1.5) * uiScale);
    }
    ...
    noStroke();
    fill(c.dark[0], c.dark[1], c.dark[2], 235);
    ellipse(0, 0, 34 * uiScale, 26 * uiScale);

    fill(c.glow[0], c.glow[1], c.glow[2], 220);
    ellipse(-6 * uiScale, -5 * uiScale, 12 * uiScale, 9 * uiScale);

    fill(c.hit[0], c.hit[1], c.hit[2], 170);
    ellipse(-20 * uiScale, 0, 24 * uiScale, 15 * uiScale);
...
window.ArrowHitEffect = class ArrowHitEffect {
  ...
  display() {
    ...
    noFill();
    stroke(c.hit[0], c.hit[1], c.hit[2], 190 * fade);
